...
- **Rule:** Use `camelCase` keys in TypeScript, mapped explicitly to `snake_case` column names in the database. For numeric primary keys, always use `generatedAlwaysAsIdentity()`. Never use `serial()`. Export all schemas via `src/database/schemas/index.ts`. When a module has multiple schema files, place them in `src/database/schemas/<module>/` with a local `index.ts`.
...
- **Bad (Negative):**
  ...
  await db.insert(roles).values([{ code: 'ADMIN', name: 'Quản trị viên' }]);
  ```
...
- **Good (Positive):**
  ...
  await db
    .insert(roles)
    .values([{ code: 'ADMIN', name: 'Quản trị viên' }])
